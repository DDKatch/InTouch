 ...
 * @author vladislav
 */
public class GetUserInfoById extends Processor{
    ...
    public JSONObject processRequest(Map<String, String[]> params) throws ServerQueryException {
        ...
        if(user==null){
            throw new ServerQueryException("User with login " + params.get("token")[0] + " does not exist");
        }
        
        JSONObject jSONObject = new JSONObject();
        jSONObject.put("result", "success");
        jSONObject.put("user", new Gson().toJson(DataHelper.getInstance().getUserById(Long.parseLong(params.get("user_id")[0])), User.class));
        return jSONObject;
    }
